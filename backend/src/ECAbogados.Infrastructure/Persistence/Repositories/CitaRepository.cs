...
public class CitaRepository(SqlConnectionFactory connectionFactory) : ICitaRepository
{
    public async Task<IReadOnlyList<Cita>> GetAllAsync()
    {
        return await ResiliencePolicies.SqlRetryPolicy.ExecuteAsync(async () =>
        {
            ...
            const string sql = """
                SELECT Id, CasoId, NombreCliente, Telefono, Email, Servicio, Modalidad, Comentario, FechaHora, Estatus, RecordatorioEnviado
                FROM dbo.Citas
                ORDER BY FechaHora
                """;
    ...
    public async Task<int> CreateAsync(Cita cita)
    {
        return await ResiliencePolicies.SqlRetryPolicy.ExecuteAsync(async () =>
        {
            ...
            const string sql = """
                INSERT INTO dbo.Citas (CasoId, NombreCliente, Telefono, Email, Servicio, Modalidad, Comentario, FechaHora, Estatus)
                OUTPUT INSERTED.Id
                VALUES (@CasoId, @NombreCliente, @Telefono, @Email, @Servicio, @Modalidad, @Comentario, @FechaHora, @Estatus)
                """;
